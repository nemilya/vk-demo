...
Стартовым файлов является файл `app.rb` в нём подгружаются требуемые библиотеки.
...
В блоке корневой страницы, происходит отрисовка EmbeddedRuby 
(html код, в который встроен Ruby) шаблона `index.erb`,
находящегося в папке 'views/'.

`erb` - это парсер, параметром является название шаблона, название шаблона
должно быть "символом" - это тип Ruby, для перевода строки в символ, можно использовать
`.to_sym`, либо перед строкой указывать ':'. То есть запись `:index` экваивалентна
"index".to_sym. Если передать `erb` в качестве параметра строку - то он воспримет 
это как html со встроенным Ruby и попытается его пропарсить.
...
Надо отметить, что перед блогом роутов, есть блок `before`:

    before do
      @app = VkontakteApi::Client.new(session[:token]) if session[:token]
    end

Этот блок в Sinatra выполяется каждый раз, перед выполнением Роутов.
